...
def aantal_kluizen_vrij():
    kluizen = open("fa_testkluizen.txt", 'r')
    aantal = 12 - len(kluizen.readlines())
    kluizen.close()
    """
...
def nieuwe_kluis():
    ...
    for i in kluizen:
        x = i.split(';')
        bezet.append(int(x[0]))
    bestand.close()
    for i in range(1,13):
        if i not in bezet:
            vrij.append(i)
    pin_c = False
    pin = input("Vul een pincode in:\n>")
    if ';' in pin or len(pin) < 4:
        return -1
    bestand = open('fa_testkluizen.txt', 'a')
    bestand.write(str(vrij[0])+';'+pin + '\n')
    bestand.close()
    return vrij[0]
...
def kluis_openen():
    in_kluis = input("Vul kluisnummer in\n>")
    in_pin = input("Vul pin in\n")
    bestand = open('fa_testkluizen.txt', 'r')
    temp = bestand.readlines()
    kluizen = []
    for i in temp:
        kluizen.append(i.split(';'))
    for i in kluizen:
        if i[0] == in_kluis and i[1].strip('\n') == in_pin:
            return True
    return False

    """
    Laat de gebruiker een kluisnummer invoeren, en direct daarna de bijbehorende
    kluiscode. Indien deze combinatie vookrkomt in het testbestand met de kluizen
    die in gebruik zijn, is het resultaat van de functie True, anders False.

    Returns:
        bdeool: True als de ingevoer combinatie correct is, anders False
    """
    return
...
def kluis_teruggeven():
    ...
    for i in temp:
        kluizen.append(i.split(';'))
    for i in kluizen:
        if not i[0] == in_kluis or not i[1].strip('\n') == in_pin:
            terug += i[0] + ';' + i[1]
        elif i[0] == in_kluis and i[1].strip('\n') == in_pin:
            x = True
    bestand.close()
    bestand = open('fa_testkluizen.txt', 'w')
    bestand.write(terug)
    print(terug)
    bestand.close
    return x
...
def development_code():
    ...
    while True:
        ...
        if keuze == '1':
            print('Er zijn', aantal_kluizen_vrij(), 'kluizen vrij.')


        elif keuze == '2':
            res = nieuwe_kluis()
            if res == -1:
                print('Fout slechte pin.')
            elif res == -2:
                print("Fout geen kluizen vrij")
            else:
                print("Jouw kluis is", res)
            ...
            if res == False:
                print("Fout combinatie bestaat niet")
            ...
            if kluis_teruggeven():
                ...
                print("Fout kluis of pin incorrect")
        elif keuze == '5':
            break
        else:
            print('Verkeerde input')
    print('Proces gestopt')
def module_runner():
